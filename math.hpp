...
#   error "your platform does not support find_highest_bit()"
	...
	template <typename B, typename I, unsigned int F, typename T, typename std::enable_if<std::is_integral<
	...
	namespace detail
	{
		...
		fixed<B, I, F> atan_sanitized(fixed<B, I, F> x) noexcept
		{
			...
			constexpr auto fC = Fixed::template from_fixed_point<63>(9178930894564541004ll);
			//  0.995181681698119  (PI/4 - A - B)
